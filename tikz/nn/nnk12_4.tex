...
\foreach \k in {2,4,6} \draw[black,-] (m-2-1) -- (m-\k-2);
\foreach \k in {2,4,6} \draw[black,-] (m-6-1) -- (m-\k-2);
...
\draw[gray,-] (m-8-2) --  (m-9-3) node[above, xshift=-4mm, yshift=1mm]{\scriptsize{$a$}}; % pwr
\draw[gray,-] (m-10-2) --  (m-9-3) node[below, xshift=-4mm, yshift=-1mm]{\scriptsize{$b$}}; % pwr
\node[gray] at (m-9-3) {\scriptsize{$a^b$}};

% connections between nodes in the third and fourth layer
\foreach \j in {2,5,9} {
\foreach \k in {2,4,6,8,10} \draw[opacity=0.2,-] (m-\j-3) -- (m-\k-4); }

\foreach \k in {2,8,10} \draw[black,-] (m-2-3) -- (m-\k-4);
