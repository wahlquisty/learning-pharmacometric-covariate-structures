...
# Training and pruning of symbolic regression network with PK simulation of Eleveld dataset (1031 patients).

# Training data:
# x: Four covariates as inputs (age (scaled 0->1), weight (scaled 0->1), gender (Male -0.5, Female 0.5), Sampling site (Arterial -0.5, Venous 0.5))
...
nepochs_init = 1000            # Nbr of epochs to train before pruning and during inputpruning
...
function symbolic_regression_nn(i, x, y, opt, lossfct, seed, nepochs_init, nepochs_prune)
    ...
    @save "bson/model$i"*"_inputpruning1.bson" model
    model = prunemodelinputs(model, N_in, x, y, verbose=false)
    model, loss_train = train!(model, x, y, nepochs_init, opt, ps, verbose=false)
    @save "bson/model$i"*"_inputpruning2.bson" model
    model = prunemodelinputs(model, N_in, x, y, verbose=false)
    model, loss_train = train!(model, x, y, nepochs_init, opt, ps, verbose=false)
    @save "bson/model$i"*"_inputpruning3.bson" model
    ...
    @save "bson/model$i"*"_parampruning1.bson" model
    ...
    for j = 2:6#1:5#11
        ...
        model, loss_train = train!(model, x, y, nepochs_prune, opt, ps, verbose=false) # train
        @save "bson/model$i"*"_parampruning$j.bson" model

    end

    N_prune = 1 # nbr of parameters to prune in each network
    for j = 7:8#1:2 # results in 12 parameters of final expression
        model = prunemodel(model, N_prune, n_params, x, y, verbose=false) # prune network parameters
...
# main(x[1:3], y[1:3], 1, opt, "ALE", seed, 1,1)
# main(x, y, 2, opt, "MdALE", seed, 1, 1)

# Run main
seed = 1234
main(x, y, 1, opt, "ALE", seed, n_epochs_init,n_epochs_prune)
